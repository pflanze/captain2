...
class PickleInitializer(StateInitializer):
    ...
    def getInitialState(self, K=None, num_species=None, length=None):
        ...
        n_cells = int(np.sqrt(len(list_cells)))
...
class EmpiricalStateInit(StateInitializer):
    def __init__(self, h3d, init_disturbance=None):
        self._h3d = h3d
        if init_disturbance is None:
            self._init_disturbance = np.zeros(h3d.shape[1:])

    def getInitialState(self, K=None, num_species=None, length=None):
        return self._h3d

    def getInitialDisturbance(self):
        return self._init_disturbance